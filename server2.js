...
const getSearchResult = function(args) {
    if (args.needle) {
        const needle = args.needle;
        return coursesData.filter(course => course.title.includes(needle));
    } else {
        return coursesData;
    }
}

const createCourse = ({ title, author, description, topic, url }) => {
    const course = {
